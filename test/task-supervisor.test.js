...
const SimpleTaskDefinition = (doneCallback = noEffect) => (
  task,
  supervisor
) => {
...
const TaskDefinition = (task, supervisor) => {
  ...
  const update_task = ({ state, msg, ctx }) => (status) => {
    const { task } = state;
...
function Supervisor(effects, makeTask = TaskDefinition, _properties) {
  ...
  const start_task = ({ state, msg, ctx }) => (task) => {
    const a_task = start_actor(ctx.self)(
      newTaskName(),
      makeTask(task, ctx.self),
      {}
    );
    dispatch(a_task, { type: "work" }, ctx.self);
    ctx.debug.trace(`Started task: ${task.taskId}`);
  };
...
context("TaskSupervisor", function() {
  ...
  describe("Effect", function() {
    ...
    it("should throw if an effect damages supervisor state", async function() {
      ...
      await deferred.promise.should.be.rejectedWith(
        TaskSupervisor.errors.EffectError
      );
    });
