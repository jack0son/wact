...
const isValidState = ({ taskRepo, tasksByStatus }) =>
  !!taskRepo && !!tasksByStatus;
...
function Actions(
  ...
  function action_submit(state, msg, ctx) {
    ...
    if (!isValidTask(_task))
      throw new Error(`New task ${taskId} is not a valid task`);

    // Ignore task if ignoreTask predicate defined
    const ignoreReason = ignoreTask && ignoreTask(_task);
    if (ignoreReason) {
      ctx.debug.d( `Ignoring task ID: ${taskId}, ${ignoreReason}`);
      return state;
    }
  ...
  async function action_update(_state, _msg, ctx) {
    ...
    if (!isStatus(_task.status)) {
      console.log(_msg);
      throw new Error(
        `Unspecified task status for taskId ${taskId}: ${_task.status}`
      );
    }
    ...
    if (_task.status === prev.status) {
      !ctx.recovering &&
        ctx.debug.trace(`Task ID: ${taskId} already has status ${_task.status.toString()}`);
      return _state;
    } else {
      ctx.debug.d(`task: ${taskId}, ${prev.status.toString()} -> ${_task.status.toString()}`);
    }
    ...
    if (isVoidState(nextState)) {
      ...
      ctx.debug.warn( `Damaged state contents: ${Object.keys(nextState)}`);
      // @TODO Should check reducer returned state as well. For now assume reducer is correct
      throw new EffectError(
        task,
        `${isEffect(effect) ? "Effect" : "Reducer" } on task status ${task.status.toString()} damaged supervisor state: ${nextState}`
      );
    }
  ...
  function action_abort(state, msg, ctx) {
    ...
    if (taskId)
      return action_update.call(ctx, state, abortMsg(taskId, "abort"), ctx);

    const statusSymbol = parseStatus(status);
    if (!statusSymbol) {
      ctx.debug.warn( `Abort expects a taskId or task status symbol`);
      return;
    }
    ...
    if (!blocking) {
      tasksByStatus[statusSymbol].values.forEach((task) =>
        dispatch(
          ctx.self,
          abortMsg(task, symbolDirectory(action_update)),
          ctx.self
        )
      );
      return;
