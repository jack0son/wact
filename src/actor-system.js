...
const { configureLogging } = require("nact/lib/monitoring");
const { block } = require("./lib/nact-utils");
const { buildReceiversHOF } = require("./receivers");
const logMessage = require("./receivers/log-message");
const action = require("./action");
const { merge } = require("./lib/utils");
const { parse_bool, configure } = require("node-utils");
 ...
 */
const bind_receivers = (receiversHof) => (state, msg, ctx) =>
  receiversHof && receiversHof({ state, msg, ctx });
...
const spawn_actor = (
  ...
  const { receivers, initialState, ...properties } = _properties; // never reference actor definition

  const _receivers = receivers || [];
  _receivers.push(logMessage());
  const inject_receivers = bind_receivers(buildReceiversHOF(_receivers));
  ...
  return spawn(
    ...
    (state, msg, context) => {
      ...
      context.receivers = inject_receivers(state, msg, context);
      context.debug = context.receivers.logMessage; // provide debug to receiver context
...
const spawn_persistent = (
  ...
  const target = debug.control.enabledByApp
    ? (state, msg, context) => {
        ...
        context.receivers = inject_receivers(state, msg, context);
        context.debug = context.receivers.logMessage;
        return action(state, msg, context);
      }
    : (state, msg, context) => {
        context.receivers = inject_receivers(state, msg, context);
...
function bootstrap(_opts = {}, _persistenceEngine) {
  const { enableLogging, consoleProxy, ...opts } = configure(_opts, {
    consoleProxy: Logger("sys", { lineNumbers: { callDepth: 2 } }),
    enableLogging: true,
  });
  let systemArgs = [opts];
  enableLogging &&
    systemArgs.push(configureLogging(logToConsole({ consoleProxy })));
  _persistenceEngine &&
    systemArgs.push(configurePersistence(_persistenceEngine));

  const system = start(...systemArgs);
  return {
